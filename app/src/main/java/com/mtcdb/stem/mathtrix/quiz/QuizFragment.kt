...
class QuizFragment : Fragment() {
    ...
    private var bonusTimeMillis : Long = 0 // Variable to store the bonus time
    ...
    private val BONUS_TIME_THRESHOLD = 5 // Threshold for correct answers to get bonus time
    ...
    private fun fetchQuestions() {
        ...
        val cursor = database.rawQuery(
            "SELECT * FROM ${QuizDatabaseHelper.TABLE_QUIZ} WHERE " +
                    "${QuizDatabaseHelper.COLUMN_DIFFICULTY_LEVEL} = ? AND " +
                    "${QuizDatabaseHelper.COLUMN_SUBJECT} = ? LIMIT ?",
